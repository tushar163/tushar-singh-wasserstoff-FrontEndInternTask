...
class SocketIOProvider {
  constructor(ydoc, socket, user, onSynced) {
    ...
    // ✅ Set the initial local awareness state
    this.awareness.setLocalState(user);
...
export default function CollaborativeEditor() {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      newProvider.awareness.on("update", ({ states }) => {
        const users = [];
        states.forEach((state) => {
          if (state && state.name) {
            users.push({ name: state.name, color: state.color });
          }
        });
...
function MenuBar({ editor }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1 p-2 border-b sticky top-0 bg-white z-10">
      <button
        onClick={() => editor.chain().focus().toggleBold().run()}
        className={`p-2 rounded hover:bg-gray-100 ${
          editor.isActive("bold") ? "bg-gray-100 font-bold" : ""
        }`}
      >
        B
      </button>
      <button
        ...
        className={`p-2 rounded hover:bg-gray-100 ${
          editor.isActive("italic") ? "bg-gray-100 italic" : ""
        }`}
      >
        I
      </button>
      <button
        ...
        className={`p-2 rounded hover:bg-gray-100 ${
          editor.isActive("underline") ? "bg-gray-100 underline" : ""
        }`}
      >
        U
      </button>
